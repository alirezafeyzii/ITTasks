	...
	public class MailService : IMailService
	{
		...
		public async Task<bool> SendMailForConfirmedAsync(MailRequestDto request)
		{
			...
			try
			{
				...
				mail.Body = new TextPart(TextFormat.Html) { Text = $"<h3>{request.UserFullName} عزیز برای فعالسازی حساب کاربری خود دکمه زیر را بفشارید </h3><a href=\"{baseUrlLocal}?userName={request.UserName}&token={request.Token}\"><button style=\"padding:10px;background-color:#6475f5;color:white;cursor: pointer;border-radius:5px;outline:none;border:none;\">فعالسازی</button></a>" };
